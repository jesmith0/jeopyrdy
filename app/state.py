...
class State:
    ...
    def update(self, input, cur_block=None, skip=False, speaking=False):
        ...
        if input or timedout:
            ...
            # DISPLAY CLUE SCREEN STATE LOGIC
                ...
                if not timedout and not self.final:
                    self.cur_state = BUZZED_STATE
                else:
            ...
            elif self.if_state(SHOW_RESP_STATE) and (
                    (not timedout and check_down) or (self.clue_timeout and active_down) or (
                    self.buzzed_timeout and buzzed_down)):
    ...
    def __check_timeout(self, skip=False):
        ...
        if self.fj_timeout:
            return True
        elif self.if_state(SHOW_CLUE_STATE) and not self.final and ((self.game_clock >= CLUE_TIMEOUT) or skip):
            return True
        ...
        elif self.if_state(BUZZED_STATE) and (self.game_clock >= BUZZ_TIMEOUT):
            return True
        elif (self.buzzed_timeout or self.clue_timeout) and not self.final:
            return True
        else:
            return False
    ...
    def __str__(self):
        ...
        if self.if_state(MAIN_STATE):
            return "MAIN STATE"
        elif self.if_state(BET_STATE):
            return "BET STATE"
        elif self.if_state(BUZZED_STATE):
            return "BUZZED STATE"
        elif self.if_state(SHOW_CLUE_STATE):
            return "SHOW CLUE STATE"
        elif self.if_state(SHOW_RESP_STATE):
            return "SHOW RESPONSE STATE"
        elif self.if_state(CHECK_STATE):
            return "CHECK RESPONSE STATE"
