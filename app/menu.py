...
class Menu:
    ...
    def __init__(self, screen, theme_channel, buzzers):
        ...
        # cursor surfaces
        self.cursor_surfaces = [gen.cursor_surface(180, 120, WHITE, "P1"), gen.cursor_surface(180, 120, WHITE, "P2"),
                                gen.cursor_surface(180, 120, WHITE, "P3"), gen.cursor_surface(180, 120, WHITE, "P4")]
    ...
    def __get_pos_from_num(self, num):
        ...
        if y_pos > 0:
            x_pos = num % (y_pos * self.charsel_dimension[0])
        else:
            x_pos = num
    ...
    def __move_cursor(self, num, dir):
        ...
        if dir == "up":
            pos[1] = pos[1] - 1 if pos[1] > 0 else num_rows + self.charsel_dimension[1]

        elif dir == "down":
            pos[1] = pos[1] + 1 if pos[1] < num_rows + self.charsel_dimension[1] else 0

        elif dir == "right":
            pos[0] = pos[0] + 1 if pos[0] < self.charsel_dimension[0] - 1 else 0

        elif dir == "left":
            pos[0] = pos[0] - 1 if pos[0] > 0 else self.charsel_dimension[0] - 1
    ...
    def __select_character(self, i):
        ...
        if not self.char_selected[i]:
            ...
            if not (self.player_cursor_pos[i] in self.active_players) or self.player_cursor_pos[i] == 0:
                ...
                # deactivate character select cursor
        ...
        if input:
            ...
            if self.charsel_state:
                ...
                for i in range(len(input)):
                    ...
                    if input[i][0]:
                        self.__select_character(i)
                    elif input[i][1]:
                        self.__move_cursor(i, "up")
                    elif input[i][2]:
                        self.__move_cursor(i, "right")
                    elif input[i][3]:
                        self.__move_cursor(i, "left")
                    elif input[i][4]:
                        self.__move_cursor(i, "down")
            ...
            # MAIN MENU
                ...
                if int(input[0][0]):
                    ...
                    # INPUT TYPE
                        ...
                        elif not self.input_state and self.BUZZERS:
                    ...
                    # PLAY MUSIC
                        ...
                        if self.music_state:
                            self.THEME_CHANNEL.set_volume(1)
                        else:
                            self.THEME_CHANNEL.set_volume(0)
    ...
    def __update_display(self):
        ...
        if self.charsel_state:
            ...
            util.blit_alpha(self.SCREEN, pygame.transform.flip(scaled_alex, True, True),
                            (DISPLAY_RES[0] - scaled_alex.get_width(), DISPLAY_RES[1] - scaled_alex.get_height()), 100)
            self.SCREEN.blit(LOGO_IMAGE, (DISPLAY_RES[0] / 2 - LOGO_IMAGE.get_width() / 2, 70))
            self.SCREEN.blit(self.gamedate_text_surface,
                             (DISPLAY_RES[0] / 2 - self.gamedate_text_surface.get_width() / 2, 275))
        ...
        for i in range(y_dim):
            for j in range(x_dim):
                ...
                for num in self.player_cursor_pos:
                    ...
                    if not self.char_selected[it]: screen.blit(self.cursor_surfaces[it],
    ...
    def __blit_all_characters(self, screen):
        ...
        for i in range(4):
            ...
            # calculate location
            blit_loc = (((width_interval * (i)) + width_interval / 2) - char_surfs[i].get_width() / 2,
