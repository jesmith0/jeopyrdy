...
class Player:
    ...
    def __init__(self, order, num):
        ...
        self.incorrect_sound = pygame.mixer.Sound(
            constants.MUSIC_PATH + "wrong" + str(num) + ".ogg") if num > 0 else None

        # blit initial score
        self.add_to_score(0)

    def get_wrong(self):
        return self.incorrect_sound

    def play_wrong(self):
        return self.incorrect_sound.play()

    def stop_wrong(self):
        self.incorrect_sound.stop()
    ...
    def get_max_bet(self, final=False):
        ...
        if self.score <= 1000 and not final:
            return 1000
        else:
            return self.score
    ...
    def inc_bet(self, final=False):
        ...
        if self.cur_bet + 100 <= self.get_max_bet(final):
            self.cur_bet += 100
        else:
            self.cur_bet = 0
        self.__update_char_surface(self.cur_bet)
    ...
    def __update_char_surface(self, value):
        ...
        self.char_surface.blit(
            gen.text_surface(value, self.char_surface.get_width(), self.char_surface.get_height(), 35, color, "digital",
                             constants.BLUE), (0, 71))
